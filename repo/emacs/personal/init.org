...
#+BEGIN_SRC emacs-lisp
  (if (string= (system-name) "odin")
      (add-to-list 'default-frame-alist '(font . "MonaspaceRadonExtraLight-16"))
    (add-to-list 'default-frame-alist '(font . "MonaspaceRadonExtraLight-16")))
#+END_SRC
** Posframe
...
#+END_SRC
*** Open Nix Repository
My develop systems are NixOS systems and so sometime i need to extend my config, here is a quick jump
#+BEGIN_SRC emacs-lisp
  (defun own/nixos-open-repo()
    (interactive)
    (projectile-find-file-in-directory "~/.nix/"))

  (global-set-key (kbd "C-c q n") 'own/nixos-open-repo)
#+END_SRC
